...
class Redis(BaseRedis):
    ...
    def get_image_captcha(self, device_id=settings.DEVICE_ID):
        # 获取图形验证码
        redis_value = self.get("LOGIN_CODE_{0}".format(device_id))
        return redis_value

    def delete_image_captcha(self, device_id=settings.DEVICE_ID):
        # 删除图形验证码
        self.remove("LOGIN_CODE_{0}".format(device_id))

    def clean_user_bean(self,user_mobile):
        # 删除用户大王豆
        user_id = MysqlOperation().get_user_id(user_mobile)
        self.remove(key='REDIS##BEAN##{0}'.format(user_id))

    def clean_user_buy_guard(self, user_mobile, anchor_id):
        # 清除用户守护信息
        user_id = MysqlOperation().get_user_id(user_mobile)
    ...
    def clean_anchor_group(self, user_mobile, anchor_id):
        # 清除主播团信息
        user_id = MysqlOperation().get_user_id(user_mobile)
    ...
    def clean_user_plat_signin(self, user_mobile):
        # 清除平台签到
        user_id = MysqlOperation().get_user_id(user_mobile)
        self.hdel(name='PLAT_SIGN_IN_USER', key=user_id)

    def clean_check_mobile_code(self, user_mobile):
        # 清除用户是否绑定手机号
        user_id = MysqlOperation().get_user_id(user_mobile)
    ...
    def set_user_plat_signin_day(self,user_mobile, day_num):
        # 修改平台签到天数
        user_id = MysqlOperation().get_user_id(user_mobile)

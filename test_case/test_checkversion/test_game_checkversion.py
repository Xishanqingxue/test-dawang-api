...
class TestGameCheckVersionApi(BaseCase):
    """
    检查电娱游戏版本
    """
    pname = 'com.dawang.entertainment'
    ...
    def test_android_force_update(self):
        ...
        :return:
        """
        sys_version_name,sys_version_code = MysqlOperation().fix_platform_upgrade(strategy=2,platform='android',pname=self.pname)
        time.sleep(1)
        ...
        db_info = MysqlOperation().get_platform_upgrade_details(channel_id=self.android_channel_id,pname=self.pname)
    ...
    def test_android_normal_update(self):
        ...
        :return:
        """
        sys_version_name, sys_version_code = MysqlOperation().fix_platform_upgrade(strategy=1,platform='android',pname=self.pname)
        time.sleep(1)
        check_version_api = CheckVersionApi()
        response = check_version_api.get({'platform': 'android', 'pname': self.pname, 'channel_id': self.android_channel_id})
    ...
    def tearDown(self,*args):
        super(TestGameCheckVersionApi,self).tearDown()
        MysqlOperation().fix_platform_upgrade(strategy=1,platform='android',up=False,pname=self.pname)
        # time.sleep()